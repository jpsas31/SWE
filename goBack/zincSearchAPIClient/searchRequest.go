...
func Search(page int, searchTerm string) ([]map[string]interface{}, int, error) {
	creds, err := LoadCredentials(SecretFilePath)
	if err != nil {
		return nil, 0, fmt.Errorf("error marshalling query: %v", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, 0, fmt.Errorf("unexpected response status: %s", resp.Status)
	}
	...
	if err != nil {
		return nil, 0, fmt.Errorf("error decoding response body: %v", err)
	}

	fmt.Println("Response Status:", resp.Status)
	totalPages := result["hits"].(map[string]interface{})["total"].(map[string]interface{})["value"].(float64)
	totalPages = math.Ceil(totalPages / pageSize)
	hits, ok := result["hits"].(map[string]interface{})["hits"].([]interface{})
	if !ok {
		return nil, 0, fmt.Errorf("error parsing response: hits.hits field is not a slice of interfaces")
	}

	return convertHitsToMaps(hits), int(totalPages), nil
}
